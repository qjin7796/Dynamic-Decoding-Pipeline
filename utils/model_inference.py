...
def inference_recognizer(model: nn.Module,
	...
	# Customized arg
	layer_name_list: a list of layer name str for layer_features
    ...
    if layer_name_list is None:
        ...
        with OutputHook(model, outputs=layer_name_list, as_tensor=False) as h:
            with torch.no_grad():
                result = model.test_step(data)[0]
            layer_features = h.layer_outputs
